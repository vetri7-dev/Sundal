...
export default function CookieConsentBanner() {
  ...
  useEffect(() => {
    ...
    // In demo mode, show on dashboard pages if not dismissed in session
    if (isDemo) {
      const dismissed = sessionStorage.getItem('cookie-banner-dismissed');
      setIsVisible(isDashboard && !dismissed);
      return;
    }
    
    // Production mode: show everywhere except landing page if no consent and logging is enabled
    if (enableLogging) {
      const consent = localStorage.getItem('cookie-consent');
      if (!consent) {
        setIsVisible(!isLandingPage);
      }
    }
  ...
  const acceptAll = async () => {
    ...
    try {
      ...
      if (isDemo) {
        // In demo mode, just mark as dismissed for the session
        sessionStorage.setItem('cookie-banner-dismissed', 'true');
      } else {
        // In production mode, save the consent
