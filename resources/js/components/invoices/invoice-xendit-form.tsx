...
interface InvoiceXenditFormProps {
  invoice: {
    id: number;
    balance_due: number;
...
export function InvoiceXenditForm({ 
  invoice, 
  xenditApiKey,
  amount,
  onSuccess, 
  onCancel 
}: InvoiceXenditFormProps) {
  ...
  const handlePayment = async () => {
    ...
    try {
      const response = await fetch(route('xendit.create-invoice-payment'), {
        method: 'POST',
        headers: {
  ...
  if (!xenditApiKey) {
    return <div className="p-4 text-center text-red-500">{t('Xendit not configured')}</div>;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-lg font-semibold">{t('Xendit Payment')}</h3>
      </div>
