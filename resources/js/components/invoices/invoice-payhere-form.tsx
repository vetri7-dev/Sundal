...
interface InvoicePayHereFormProps {
  invoice: any;
  amount: number;
  onSuccess: () => void;
  onCancel: () => void;
...
export function InvoicePayHereForm({ 
  invoice, 
  amount, 
  onSuccess, 
  onCancel,
  payhereMerchantId 
}: InvoicePayHereFormProps) {
  ...
  const handlePayment = async () => {
    ...
    try {
      const response = await fetch(route('payhere.create-invoice-payment'), {
        method: 'POST',
        headers: {
    ...
    <Card>
      <CardContent className="p-6 space-y-4">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-semibold">{t('PayHere Payment')}</h3>
        </div>
        ...
        <div className="flex gap-3 pt-4">
          ...
          <Button
            ...
            disabled={loading || !payhereMerchantId}
            className="flex-1"
          >
            {loading ? (
              <>
        ...
        <div className="text-xs text-muted-foreground text-center">
          {t('Powered by PayHere - Sri Lankan payment gateway')}
        </div>
      </CardContent>
    </Card>
  );
