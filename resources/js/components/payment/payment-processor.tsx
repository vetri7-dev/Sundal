...
export function PaymentProcessor({ 
  ...
  const originalPrice = billingCycle === 'yearly' && plan.yearly_price ? Number(plan.yearly_price) : Number(plan.price);
    ...
    <div className="space-y-6">
        ...
        <CardContent className="p-4">
          <div className="flex justify-between items-center">
            ...
            <div className="text-right">
              <div className="text-lg font-bold">{(() => { if (typeof window !== "undefined" && window.appSettings?.formatCurrency) { return window.appSettings.formatCurrency(originalPrice); } return originalPrice.toFixed(2); })()}</div>
        ...
        <CardContent className="p-4">
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span>{(() => { if (typeof window !== "undefined" && window.appSettings?.formatCurrency) { return window.appSettings.formatCurrency(originalPrice); } return originalPrice.toFixed(2); })()}</span>
            </div>
            {appliedCoupon && (
              <div className="flex justify-between text-sm text-green-600">
                <span>{t('Discount')}</span>
                <span>-{(() => { if (typeof window !== "undefined" && window.appSettings?.formatCurrency) { return window.appSettings.formatCurrency(discountAmount); } return discountAmount.toFixed(2); })()}</span>
              </div>
            )}
            <div className="border-t pt-2">
              <div className="flex justify-between font-medium">
                <span>{t('Total')}</span>
                <span>{(() => { if (typeof window !== "undefined" && window.appSettings?.formatCurrency) { return window.appSettings.formatCurrency(finalPrice); } return finalPrice.toFixed(2); })()}</span>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <div className="flex gap-3">
        ...
        <Button 
          ...
          className="flex-1"
        >
          {t('Pay')} {(() => { if (typeof window !== "undefined" && window.appSettings?.formatCurrency) { return window.appSettings.formatCurrency(finalPrice); } return finalPrice.toFixed(2); })()}
        </Button>
      </div>
    </div>
  );
