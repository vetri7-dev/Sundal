...
export default function ManageLanguagePage() {
  ...
  const { languages, defaultLang, defaultData, availableLanguages } = usePage<PageProps>().props;
  ...
  const handleSave = (e) => {
    ...
    .then(data => {
      if (!data) return; // Already handled 403 case above
  ...
  const handleDeleteLanguage = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast.error(data.message || data.error || t('Failed to delete language'));
      }
    } catch (error) {
  ...
  const handleToggleLanguage = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (!isCurrentLanguageEnabled || selectedLang !== 'en') {
          router.get(route('manage-language', { lang: 'en' }));
        } else {
          router.get(route('manage-language', { lang: selectedLang }));
        }
      } else {
        toast.error(data.message || data.error || t('Failed to update language status'));
      }
    } catch (error) {
  ...
  const currentLang = availableLanguages?.find(lang => lang.code === selectedLang);
  const isCurrentLanguageEnabled = currentLang?.enabled ?? true;
  ...
  return (
    ...
    >
              ...
              <div className="pr-4 space-y-1">
                {availableLanguages?.map((lang) => (
                  ...
                  >
                    ...
                    <span className={lang.enabled === false ? 'text-muted-foreground' : ''}>
                      {lang.name}
                    </span>
                    {lang.enabled === false && (
          ...
          <Card className="p-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-2">
              <h2 className="text-lg font-semibold">
                ...
                {t('Edit Labels for')} {(availableLanguages || languages).find(l => l.code === selectedLang)?.name}
              </h2>
              <Input
      ...
      <CreateLanguageModal
        ...
        onSuccess={() => {
          // Refresh the page to show the new language
